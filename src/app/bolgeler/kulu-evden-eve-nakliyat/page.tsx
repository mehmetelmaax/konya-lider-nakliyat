...
export const metadata: Metadata = {
  title: 'Kulu Evden Eve Nakliyat | Lider Nakliyat',
  description: "Konya Kulu ilçesinde yurt dışı dönüş tır yüklemeleri dahil sigortalı ve marangozlu ev taşıma.",
  alternates: {
    canonical: '/bolgeler/kulu-evden-eve-nakliyat',
  },
  robots: {
    index: false,
    follow: true,
...
export default function KuluPage() {
  const sss = [
    {
  ...
  const schemas = {
    ...
    '@graph': [
      serviceSchema({
        name: 'Kulu Evden Eve Nakliyat',
        description: "Konya Kulu ilçesinde yurt dışı dönüş tır yüklemeleri dahil sigortalı ve marangozlu ev taşıma.",
        slug: 'bolgeler/kulu-evden-eve-nakliyat',
        areaName: 'Kulu'
      }),
      breadcrumbSchema([
        { name: 'Ana Sayfa', url: '/' },
        { name: 'Bölgelerimiz', url: '/bolgeler' },
        { name: 'Kulu', url: '/bolgeler/kulu-evden-eve-nakliyat' }
      ]),
      faqSchema(sss)
    ]
  };

  const mahalleler = ["Kemaliye","Karşıyaka","Yeni Mahalle","Fatih","Cumhuriyet","Alparslan"];
  ...
  return (
    <>
      ...
      <main className="pt-24 bg-off-white">
        <Breadcrumb items={[{ name: 'Bölgelerimiz', url: '/#ilcelerimiz' }, { name: 'Kulu', url: '/bolgeler/kulu-evden-eve-nakliyat' }]} className="pt-4" />
        ...
        <section className="py-20 bg-navy text-white text-center space-y-4">
          ...
          <p className="text-gray-300 text-sm md:text-base max-w-2xl mx-auto leading-relaxed px-4">
            Kulu ilçesinin tüm mahallelerinde K3 belgeli kapalı kasa araçlarımız, marangozlu nakliye ekiplerimiz ve dış cephe asansörlerimizle sabit fiyat garantili profesyonel taşıma çözümleri sunuyoruz.
          </p>
        </section>
        ...
        <section className="py-20 max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 space-y-12">
          ...
          <div className="bg-white p-8 rounded-xl border border-gray-light shadow-sm">
            ...
            <QuoteForm defaultDistrict="Kulu" />
          </div>
